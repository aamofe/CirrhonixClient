  ...
  <div class="card manual-crawler">
    ...
    <div class="query-section">
      <div class="form-group">
        ...
        <el-input id="keywords" v-model="manualCrawler.keywords" placeholder="输入检索关键词" clearable />
      </div>

      <div class="form-row">
        <div class="form-group flex-1">
          <label for="start-date">起始日期:</label>
          <el-date-picker id="start-date" v-model="manualCrawler.startDate" type="date"
            value-format="YYYY-MM-DD" placeholder="选择日期" class="full-width" />
        </div>

        <div class="form-group flex-1">
          <label for="end-date">结束日期:</label>
          <el-date-picker id="end-date" v-model="manualCrawler.endDate" type="date"
            value-format="YYYY-MM-DD" placeholder="选择日期" class="full-width" />
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <el-input-number id="max-results" v-model="manualCrawler.maxResults" :min="1" :max="500" class="full-width" />
      </div>
    </div>
    ...
    async restoreTaskFromMemory() {
      ...
      if (savedTaskId) {
        try {
          ...
          notify.warning('无法恢复之前的任务，可能任务已过期')
        }
      } else {
        try {
    ...
    async startManualCrawling() {
      ...
      try {
        ...
        if (response.data && response.data.task) {
          ...
          notify.success("爬取任务已创建，正在获取结果...")
          this.startPolling(this.currentTask.id)
        } else {
          throw new Error("创建任务失败")
        }
      } catch (error) {
        notify.apiError(error, "启动爬取任务失败")
      } finally {
        ...
        if (response.data) {
          ...
          if (['completed', 'failed'].includes(this.currentTask.status)) {
            ...
            if (this.currentTask.status === 'completed') {
              ...
              notify.success(`爬取任务完成，共获取 ${count} 篇文献`)
            } else if (this.currentTask.status === 'failed') {
              notify.error("爬取任务失败: " + (this.currentTask.error_message || "未知错误"))
            }
          }
        }
      } catch (error) {
